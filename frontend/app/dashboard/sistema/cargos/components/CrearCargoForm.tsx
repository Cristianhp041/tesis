...
import { CREATE_CARGO } from "../graphql/createCargo";
...
export default function CrearCargoForm({ onSuccess, onCancel }: Props) {
  const [nombre, setNombre] = useState("");

  const [createCargo, { loading }] = useMutation(CREATE_CARGO, {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        if (errorMessage.includes("Ya existe un cargo con ese nombre")) {
