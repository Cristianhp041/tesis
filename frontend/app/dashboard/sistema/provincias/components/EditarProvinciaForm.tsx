...
import { UPDATE_PROVINCIA } from "../graphql/updateProvincia";
...
type UpdateProvinciaInput = {
  nombre?: string;
};

export default function EditarProvinciaForm({
  provincia,
  onSuccess,
  onCancel,
}: Props) {
  ...
  const [updateProvincia, { loading }] = useMutation(UPDATE_PROVINCIA, {
    refetchQueries: [{ query: GET_PROVINCIAS }],
  });

  const hayCambios = nombre !== original.nombre;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data: UpdateProvinciaInput = {};

    if (nombre !== original.nombre) data.nombre = nombre;

    try {
      const result = await updateProvincia({
        variables: {
          id: provincia.id,
          data,
        },
