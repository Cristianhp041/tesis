...
import { UPDATE_SUBCLASIFICACION } from "../graphql/updateSubclasificacion";
...
type UpdateSubclasificacionInput = {
  nombre?: string;
  activo?: boolean;
};

export default function EditarSubclasificacionForm({
  sub,
  onSuccess,
  onCancel,
}: Props) {
  const [nombre, setNombre] = useState(sub.nombre);
  const [activo, setActivo] = useState(sub.activo);
  ...
  const [updateSubclasificacion, { loading }] = useMutation(
    UPDATE_SUBCLASIFICACION,
    {
  ...
  const hayCambios =
    nombre !== original.nombre || activo !== original.activo;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.error) {
        ...
        } else if (errorMessage.includes("Ya existe una subclasificación con ese nombre")) {
      ...
      toast.success("✓ Subclasificación actualizada correctamente");
      onSuccess();
    } catch (error) {
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-gray-500">
            No se podrá usar si está inactiva
          </p>
        </div>
